...
class TradingBot:
    ...
    def get_trade_params(self, signal):
        try:
            # symbol_params = get_symbol_params(self.symbol)
            symbol_params = self.get_symbol_params()
            min_distance = symbol_params['stops_level'] * symbol_params['point'] * 1.5  # Margem de segurança
            ...
            atr = self.calculate_atr() or (20 * symbol_params['point'])  # Fallback baseado no ponto

            # Cálculo inicial de SL/TP
            if signal == 1:
            ...
            if abs(price - sl) < min_distance:
                sl_adjustment = min_distance * 1.2
                sl = price - sl_adjustment if signal == 1 else price + sl_adjustment

            if abs(price - tp) < min_distance:
                tp_adjustment = min_distance * 2.5
                tp = price + tp_adjustment if signal == 1 else price - tp_adjustment

            # Arredondamento para casas decimais corretas
            sl = round(sl, symbol_params['digits'])
            tp = round(tp, symbol_params['digits'])
            price = round(price, symbol_params['digits'])

            return (price, tp, sl, self.calculate_volume(atr, symbol_params))
    ...
    def calculate_volume(self, atr, symbol_params):
        try:
            # 1. Obter informações da conta com fallback
            account_info = mt5.account_info()
            balance = account_info.balance if account_info else SALDO_INICIAL_TREINO

            # 2. Calcular risk_amount com limites
            risk_per_trade = 0.01  # Definir explicitamente
            risk_amount = min(balance * risk_per_trade, balance * 0.05)  # Máximo 5% do saldo
            ...
            # 4. Cálculo seguro do stop_loss_pips
            pip_value = 10  # Valor fixo para AUDCAD (ajuste conforme o par)
            stop_loss_pips = (atr * ATR_SL_MULTIPLIER) / symbol_params['point'] if atr else 30

            # ✅ Novo ajuste: garantir distância mínima de 54 pontos
            stop_loss_pips = max(stop_loss_pips, 54)
            ...
            # 6. Respeitar os limites de volume da corretora
            volume = max(
            ...
            print(f"⚖️ Volume calculado: {volume:.2f} | Risk: ${risk_amount:.2f} | SL_pips: {stop_loss_pips:.2f}")
            return volume

        except Exception as e:
            print(f"🚨 Erro crítico no cálculo de volume: {str(e)}")
            return 0.0  # Retornar 0 em caso de falha
        ...
        try:
            RL_MODEL = DQN.load(
                ...
                custom_objects={
                    ...
                    "policy_kwargs": dict(features_extractor_class=TemporalAttentionExtractor)
                },
                device='auto'
            )
...
async def main():
    ...
    # Conexão inicial robusta
    for _ in range(5):  # Tentar 5 vezes
        if mt5.initialize():
            break
        print(Fore.RED + "❌ Falha na conexão inicial. Tentando novamente em 5s...")
        await asyncio.sleep(5)
    else:
    ...
    if not mt5.initialize() or not mt5.account_info():
        print(Fore.RED + "❌ Não foi possível estabelecer conexão!")
        return
    ...
    if choose_training_mode():
        ...
        if os.path.exists(model_path):
            RL_MODEL = MetaDQN.load(
                ...
                env=env,
                policy=CustomDQNPolicy,  # ← Especifique a política aqui
                device='auto'
            )
            # RL_MODEL = MetaDQN.load(
            #     "dqn_trading_agent_best.zip",
            #     env=env,
            #     custom_objects={"policy": CustomDQNPolicy},
            #     device='auto'
            # )
            # RL_MODEL = MetaDQN.load(
            #     "dqn_trading_agent_best.zip",
